...
function ispapi_create_client ( $contact ) {
	...
	if ( !strlen($info["postcode"]) ) $info["postcode"] = "NONE";
	if ( !strlen($info["state"]) ) $info["state"] = "N/A";
...
function ispapi_get_domainprices ( $currencyid ) {
	...
	while ($row = @mysql_fetch_array ($query, MYSQL_ASSOC)) {
		if ($row['year1'] != 0) $domainprices[$row['extension']][$row['type']][1] = $row['year1'];
		if ($row['year2'] != 0) $domainprices[$row['extension']][$row['type']][2] = $row['year2'];
		if ($row['year3'] != 0) $domainprices[$row['extension']][$row['type']][3] = $row['year3'];
		if ($row['year4'] != 0) $domainprices[$row['extension']][$row['type']][4] = $row['year4'];
		if ($row['year5'] != 0) $domainprices[$row['extension']][$row['type']][5] = $row['year5'];
		if ($row['year6'] != 0) $domainprices[$row['extension']][$row['type']][6] = $row['year6'];
		if ($row['year7'] != 0) $domainprices[$row['extension']][$row['type']][7] = $row['year7'];
		if ($row['year8'] != 0) $domainprices[$row['extension']][$row['type']][8] = $row['year8'];
		if ($row['year9'] != 0) $domainprices[$row['extension']][$row['type']][9] = $row['year9'];
		if ($row['year10'] != 0) $domainprices[$row['extension']][$row['type']][10] = $row['year10'];
	}
	return $domainprices;
}
